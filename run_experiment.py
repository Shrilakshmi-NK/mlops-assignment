...
os.environ['MLFLOW_TRACKING_URI'] = 'https://dagshub.com/Shrilakshmi-NK/mlops-assignment.mlflow'
os.environ['MLFLOW_TRACKING_USERNAME'] = 'Shrilakshmi-NK'
os.environ['MLFLOW_TRACKING_PASSWORD'] = 'b9998ddddb6293b3f94dafb8e973620bc829ae2c'  # Your token

mlflow.set_tracking_uri(os.environ['MLFLOW_TRACKING_URI'])
mlflow.set_experiment("cpu-usage-comparison")
...
def train_and_log(model, model_name, params):
    ...
    with mlflow.start_run(run_name=f"{model_name}-v1"):
        for k, v in params.items():
            mlflow.log_param(k, v)
        mlflow.log_metric("mse", mse)
        mlflow.log_metric("r2", r2)
        # Comment out model log if error (DagsHub limit)
        # mlflow.sklearn.log_model(pipeline, "model")
        # Save locally
...
train_and_log(LinearRegression(), "LinearRegression", {"model": "LinearRegression"})

# Model 2: Random Forest
train_and_log(RandomForestRegressor(n_estimators=100, max_depth=15, random_state=42), "RandomForest", {"model": "RandomForest", "n_estimators": 100, "max_depth": 15})

print("All models trained! Check DagsHub Experiments tab for comparison graphs.")